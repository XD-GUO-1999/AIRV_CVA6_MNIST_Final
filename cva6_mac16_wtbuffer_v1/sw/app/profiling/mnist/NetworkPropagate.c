/*
 * NetworkPropagate_profile.c
 *
 * HOST-ONLY gprof profiling version.
 *
 * Compile on the Linux host with -O0 -pg -g -fno-inline.
 * It does NOT execute the RISC-V custom instructions.
 * Instead, every logical custom instruction calls:
 *   prof_mac16buf()
 *   prof_mac16buf_para()
 *   prof_buf4()
 *
 * This lets gprof "calls" report dynamic custom-instruction counts.
 */
...
static __attribute__((noinline)) int clamp(int v, int lo, int hi) {
    if(v < lo) {
        return lo;
    }
    else if(v > hi) {
...
/*
 * ============================================================
 * HOST GPROF PROFILING HELPERS
 * ============================================================
 *
 * This file is intended ONLY for host-side gprof profiling.
 * RISC-V custom instructions are replaced by calls to the three
 * noinline functions below.
 *
 * Therefore:
 *   prof_mac16buf()      calls == dynamic MAC16BUF instruction count
 *   prof_mac16buf_para() calls == dynamic MAC16BUF_PARA instruction count
 *   prof_buf4()          calls == dynamic BUF4 instruction count
 *
 * The MAC arithmetic itself is NOT emulated here. The objective
 * is only to preserve the program control flow and count calls.
 */

static volatile unsigned long prof_sink = 0;

static __attribute__((noinline)) void prof_mac16buf(void)
{
    prof_sink++;
}

static __attribute__((noinline)) void prof_mac16buf_para(void)
{
    prof_sink++;
}

static __attribute__((noinline)) void prof_buf4(void)
{
    prof_sink++;
}
...
/*
 * ============================================================
 * HOST-ONLY replacements for RISC-V custom-instruction helpers
 * ============================================================
 */

static __attribute__((noinline)) void buffer4_setmode_conv1(void)
{
    prof_buf4();
}

static __attribute__((noinline)) void buffer4_setmode_conv2(void)
{
    prof_buf4();
}

static __attribute__((noinline)) void buffer4_setmode_fc1(void)
{
    prof_buf4();
}

static __attribute__((noinline)) void buffer4_setmode_fc2(void)
{
    prof_buf4();
}


/* ---------- Conv1 PARA variants: one helper call = one MAC16BUF_PARA ---------- */
...
static __attribute__((noinline))
void mac16buf_wbuf_middle(void)
{
    prof_mac16buf();
}
...
/* ---------- middle4 contains FOUR real MAC16BUF instructions ---------- */

static __attribute__((noinline))
void mac16buf_middle4(
    const WDATA_T* __restrict weights)
{
    prof_mac16buf();
    prof_mac16buf();
...
static __attribute__((noinline))
void mac16buf_wbuf_middle4(void)
{
    prof_mac16buf();
    prof_mac16buf();
    prof_mac16buf();
...
/* ---------- PARA middle4 contains FOUR real MAC16BUF_PARA instructions ---------- */
...
/* ---------- FC2 offset-2 weight-loading variants ---------- */
...
/* ---------- Single MAC16BUF_PARA helpers ---------- */

static __attribute__((noinline))
void mac16buf_para_first(
    const UDATA_T* __restrict inputs,
    const WDATA_T* __restrict weights,
    SUM_T* __restrict weightedSum)
{
...
/*
 * Higher-level block helpers are kept as real functions so gprof
 * can also show how often these helpers themselves are called.
 */
...
void mac16buf_conv2_25blocks(
    ...
    SUM_T* __restrict weightedSum)
{
    /* 25 real MAC16BUF instructions */
    mac16buf_first(weights + 0, weightedSum);
    ...
    mac16buf_final(weights + 384, weightedSum); /* block 24 */
}

static __attribute__((noinline))
void mac16buf_wbuf_conv2_25blocks(
    SUM_T* __restrict weightedSum)
{
    /* 25 real MAC16BUF instructions */
    mac16buf_wbuf_first(weightedSum);

    mac16buf_wbuf_middle4();
    mac16buf_wbuf_middle4();
    mac16buf_wbuf_middle4();
    mac16buf_wbuf_middle4();
    mac16buf_wbuf_middle4();

    mac16buf_wbuf_middle();
    mac16buf_wbuf_middle();
    mac16buf_wbuf_middle();

    mac16buf_wbuf_final(weightedSum);
}
    ...
    SUM_T* __restrict weightedSum)
{
    ...
    mac16buf_final(weights + 368, weightedSum); /* 23 */
}
...
static __attribute__((noinline)) UDATA_T saturate(SUM_T value, uint32_t sat) {
    return clamp(value, (SUM_T)(0), ((SUM_T)(1) << sat) - 1);
}

static __attribute__((noinline)) UDATA_T sat(SUM_T weightedSum, int output,
             ...
             */
            {
                ...
                if (capture_weights) {
                    ...
                    if (input_alignment == 0u) {
                        ...
                        * 每行直接使用一个lw。
                        */
                        weightedSum = mac16buf_para_conv1_aligned(
                            row0,
                            row1,
                            row2,
                            row3,
                        ...
                        * 每行使用两个lhu拼接。
                        */
                        weightedSum = mac16buf_para_conv1_unaligned2(
                            row0,
                            row1,
                            row2,
                            row3,
                    ...
                    if (input_alignment == 0u) {
                        ...
                        * 每行直接使用一个lw。
                        */
                        weightedSum = mac16buf_para_conv1_aligned_wbuf(
                            row0,
                            row1,
                            row2,
                            row3,
                        ...
                        * 每行使用两个lhu拼接。
                        */
                        weightedSum = mac16buf_para_conv1_unaligned2_wbuf(
                            row0,
                            row1,
                            row2,
                            row3,
