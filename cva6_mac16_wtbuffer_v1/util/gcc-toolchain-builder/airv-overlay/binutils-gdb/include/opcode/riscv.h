...
#define EXTRACT_RVV_VC_IMM(x) \
  (RV_X(x, 20, 11))
/*
 * AIRV custom GPR operand fields.
 *
 * The custom accelerator instructions keep rd in bits [11:7] and use the
 * remaining four 5-bit fields as register specifiers W1-W4:
 *   W1 -> instruction[16:12]
 *   W2 -> instruction[21:17]
 *   W3 -> instruction[26:22]
 *   W4 -> instruction[31:27]
 *
 * GAS parses these operands through the W1-W4 syntax defined in tc-riscv.c.
 */
#define EXTRACT_AIRV_W1(x) \
  (((x) >> 12) & 0x1f)
#define EXTRACT_AIRV_W2(x) \
  (((x) >> 17) & 0x1f)
#define EXTRACT_AIRV_W3(x) \
  (((x) >> 22) & 0x1f)
#define EXTRACT_AIRV_W4(x) \
  (((x) >> 27) & 0x1f)

#define ENCODE_AIRV_W1(x) \
  (((x) & 0x1f) << 12)
#define ENCODE_AIRV_W2(x) \
  (((x) & 0x1f) << 17)
#define ENCODE_AIRV_W3(x) \
  (((x) & 0x1f) << 22)
#define ENCODE_AIRV_W4(x) \
  (((x) & 0x1f) << 27)
