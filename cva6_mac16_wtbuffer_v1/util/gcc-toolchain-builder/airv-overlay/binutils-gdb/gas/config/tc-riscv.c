...
validate_riscv_insn (const struct riscv_opcode *opc, int length)
{
  ...
  for (oparg = opc->args; *oparg; ++oparg)
	...
	case 'O': /* Opcode for .insn directive.  */
	  ...
	  break;
    /* AIRV custom GPR operands W1-W4.
       Mark the non-standard register fields as used during opcode validation.  */
    case 'W':
	  switch (*++oparg)
	    {
	      case '1': used_bits |= ENCODE_AIRV_W1 (-1U); break;
	      case '2': used_bits |= ENCODE_AIRV_W2 (-1U); break;
	      case '3': used_bits |= ENCODE_AIRV_W3 (-1U); break;
	      case '4': used_bits |= ENCODE_AIRV_W4 (-1U); break;
	      default:
		goto unknown_validate_operand;
	    }
	  break;
	  ...
	  bfd_reloc_code_real_type *imm_reloc, htab_t hash)
{
  ...
  for ( ; insn && insn->name && strcmp (insn->name, str) == 0; insn++)
	...
	case 'W':
	      /* AIRV custom register operand.
		 W1-W4 are normal GPR names encoded in instruction[16:12],
		 [21:17], [26:22], and [31:27], respectively.  */
	      switch (*++oparg)
		{
		case '1':
		  if (!reg_lookup (&asarg, RCLASS_GPR, &regno)) break;
		  ip->insn_opcode |= ENCODE_AIRV_W1 (regno);
		  continue;
		case '2':
		  if (!reg_lookup (&asarg, RCLASS_GPR, &regno)) break;
		  ip->insn_opcode |= ENCODE_AIRV_W2 (regno);
		  continue;
		case '3':
		  if (!reg_lookup (&asarg, RCLASS_GPR, &regno)) break;
		  ip->insn_opcode |= ENCODE_AIRV_W3 (regno);
		  continue;
		case '4':
		  if (!reg_lookup (&asarg, RCLASS_GPR, &regno)) break;
		  ip->insn_opcode |= ENCODE_AIRV_W4 (regno);
		  continue;
		default:
		  goto unknown_riscv_ip_operand;
		}
