	  ...
	  switch (*++oparg)
	    {
	      ...
	      case '2': used_bits |= ENCODE_MAC8_RS2 (-1U); break;
	      case '3': used_bits |= ENCODE_MAC8_RS3 (-1U); break;
	      case '4': used_bits |= ENCODE_MAC8_RS4 (-1U); break;
	      default:
		goto unknown_validate_operand;
	    }
	  break;
...
//The operand is encoded in the same way as normal GPRs, but goes in a different place in the instruction encoding, so we need to handle it specially here.
		...
		case '1':
		  ...
		  ip->insn_opcode |= ENCODE_MAC8_RS1 (regno);
		  continue;
		case '2':
		  if (!reg_lookup (&asarg, RCLASS_GPR, &regno)) break;
		  ip->insn_opcode |= ENCODE_MAC8_RS2 (regno);
		  continue;
		case '3':
		  if (!reg_lookup (&asarg, RCLASS_GPR, &regno)) break;
		  ip->insn_opcode |= ENCODE_MAC8_RS3 (regno);
		  continue;
		case '4':
		  if (!reg_lookup (&asarg, RCLASS_GPR, &regno)) break;
		  ip->insn_opcode |= ENCODE_MAC8_RS4 (regno);
		  continue;
		default:
		  goto unknown_riscv_ip_operand;
		}
