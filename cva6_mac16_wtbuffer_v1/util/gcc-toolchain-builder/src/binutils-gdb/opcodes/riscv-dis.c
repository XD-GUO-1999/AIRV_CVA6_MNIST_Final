...
#undef DECLARE_CSR
	...
	case 'Z':
	  ...
	  break;
//modification for custom extensions
case 'W': /* 反汇编 mac8ex 自定义寄存器 */
//here we assume that the custom registers are encoded in the same way as general-purpose registers, 
//and we use the same register names for simplicity. In a real implementation, 
//you would likely have a separate array of register names for the custom registers.
	  switch (*++oparg)
	    {
	    case '1':
	      print (info->stream, dis_style_register, "%s", riscv_gpr_names[EXTRACT_MAC8_RS1 (l)]);
	      break;
	    case '2':
	      print (info->stream, dis_style_register, "%s", riscv_gpr_names[EXTRACT_MAC8_RS2 (l)]);
	      break;
	    case '3':
	      print (info->stream, dis_style_register, "%s", riscv_gpr_names[EXTRACT_MAC8_RS3 (l)]);
	      break;
	    case '4':
	      print (info->stream, dis_style_register, "%s", riscv_gpr_names[EXTRACT_MAC8_RS4 (l)]);
	      break;
	    default:
	      print (info->stream, dis_style_text, "?");
	      break;
